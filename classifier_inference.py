...
def classifier_inference(test_data, byol, classifier, byol_time, classifier_time ,sigma=0.1 ,batch_size=256 ,noisy_inference=False, write_to_log=True):
...
def save_imgs(test_data, byol, classifier ,batch_size=8, sigma=0.1):            
    ...
    fig.savefig(f'images_with_predictions_sigma={sigma}.png')
...
def plot_tsne(X, y, N_samples, dim=2, perplexity=30.0, sigma=0.2):
    ...
    if dim == 2:
        ...
        for l in range(10):
            ax.scatter(orig_embedded[y==l,0], orig_embedded[y==l, 1], color=colors[l], marker='.', label=f'class={labels_to_classes[l]}')
        ...
        for l in range(10):
            ax.scatter(orig_embedded[y==l,0], orig_embedded[y==l, 1], orig_embedded[y==l, 2], color=colors[l], marker='.', label=f'class={labels_to_classes[l]}')
            ax.scatter(noisy_embedded[y==l,0], noisy_embedded[y==l, 1], noisy_embedded[y==l, 2], color=colors[l], marker='*', label='Noisy')
        ax.grid()
        ax.legend(bbox_to_anchor=(1, 1), loc="upper left")
        ax.set_title("3D t-SNE of the embedded images")
    
    plt.savefig(f'tsne_{dim}D_on_embedding_sigma={sigma}', bbox_inches="tight")
